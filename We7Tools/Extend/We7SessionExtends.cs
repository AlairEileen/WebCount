...
namespace We7Tools.Extend
{
    public static class We7SessionExtends
    {
        ...
        public static string GetReferrer(this Microsoft.AspNetCore.Http.HttpContext httpContext)
        {
            byte[] jObj;
            if (!httpContext.Session.TryGetValue($"{We7Config.We7ProjName}referer", out jObj))
            {
                return null;
            }
            return jObj == null ? null : Encoding.UTF8.GetString(jObj);
        }
        public static void SetReferrer(this Microsoft.AspNetCore.Http.HttpContext httpContext, string referrer)
        {
            httpContext.Session.Set($"{We7Config.We7ProjName}referer", Encoding.UTF8.GetBytes(referrer));
        }
    }
